...
String InBytes = "";    // define a string to hold received serial characters
...
void setup() {
  Serial.begin(9600);    // Open a serial channel to Arduino at a baudrate of 9600
...
int forward = setSpeed(120)
  ...
  if (Serial.available()) {  // If serial data is available...
    ...
    inBytes += char(inChar);// Join the latest character to a growing string of recent chracters
  
  if (serial.read = serial.write){
    if(inChar = '1'){
      Lmotor.setSpeed(120)
